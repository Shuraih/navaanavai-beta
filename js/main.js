...
function contentToggle($this){
  ...
  var self    = $this,
    ...
    container = $('.marshall-container').first();

    console.log(container);

    console.log(details);

  if ( details.hasClass('mrs-active') ) {
    close.hide(0);
    // self.removeClass('marshall-animate-open');
    // content.removeClass('mrs-scaleDown').addClass(animation).addClass('mrs-active');
    details.removeClass('mrs-active');
    console.log('clicked');
  } else {
    close.delay(600).show(0);
    container.css('width', window.outerWidth / 2);
    container.addClass('information-open');
    $(window).trigger('resize');
    $('.navaanavai-details').css({
      left: "-" + (parseFloat($('.marshall-col-content').css('margin-left')) - ($('.navaanavai-details').position().left)) + "px"
    });
...
$(document).on("click", "#marshall-close-content", function(e){
  ...
  var self    = $(this),
    ...
    $(".marshall-container").attr('style', '').removeClass('information-open');

    $('.navaanavai-details').css({
      left: (Math.abs($('.navaanavai-details').position().left)) + "px"
    });
