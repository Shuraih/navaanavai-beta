...
// var size = Math.floor(document.body.clientWidth / 2); // 50% window width

// bodyWidth  = document.body.clientWidth * 0.8;
//   bodyHeight = (document.body.clientWidth * 0.8) + size;

var initThree = function() {
  canvas = document.getElementById('wriggle-shape');
...
var init = function() {
  var ballGeometry = new THREE.SphereGeometry(ballRadius, 20, 20);
  ...
  light = new HemiLight();
  light.init(scene, get.radian(100), get.radian(200), 1000, 0xff6666, 0xa33, 1);
...
var computeScreenSpaceBoundingBox = function(mesh, camera) {
  ...
  for (var i = 0; i < vertices.length; i++) {
    ...
    var vertexScreenSpace = vertexWorldCoord.project(camera.obj);
...
var normalizedToPixels = function(coord, renderWidthPixels, renderHeightPixels) {
...
var alignObjects = function() {
  ...
  var boundingBox2D = computeScreenSpaceBoundingBox(ball.mesh, camera);

  // console.log(boundingBox2D);

  // Convert normalized screen coordinates [-1, 1] to pixel coordinates:
  var pixelCoordScale = normalizedToPixels(boundingBox2D.size(), bodyWidth, bodyHeight); 
  var pixelCoordCenter = normalizedToPixels(boundingBox2D.center(), bodyWidth, bodyHeight);

  // console.log(pixelCoordScale);
  // console.log(pixelCoordCenter);

  var ball_atop = ((bodyHeight / 2) - (pixelCoordScale.y / 2)) + $('#wriggle-shape canvas').position().top;
  var ball_aleft = ((bodyWidth / 2) - (pixelCoordScale.x / 2)) + $('#wriggle-shape canvas').position().left;

  if(container.hasClass('information-open')) {
    console.log('information-open');
    ball_aleft = ((container.width() / 2) - (pixelCoordScale.x / 2)) + $('#wriggle-shape canvas').position().left;
  } 

  ball_aleft = ball_aleft  - parseFloat($('.marshall-col-content').css('margin-left'));

  if(container.hasClass('information-open')) {
    ball_aleft = "-" + (ball_aleft / 2) + "px";
    console.log(ball_aleft);
  } 

  console.log(ball_aleft);
  console.log($('.marshall-col-content').position().left);

  $('.navaanavai-details').css({
    position: 'absolute',
    width: pixelCoordScale.x,
    top: ball_atop,
    left: ball_aleft
  });

  $("#logo-text").css({
    'width': pixelCoordScale.x * 0.6,
  });

  console.log($("#logo-text").height());
  console.log((pixelCoordScale.y - $("#logo-text").height()) / 2);

  $("#logo-text").css({
    'padding-top': (pixelCoordScale.y - $("#logo-text").height()) / 2,
    'padding-bottom': 40 + ((pixelCoordScale.y - $("#logo-text").height()) / 2),
  });
...
var resizeRenderer = function() {
  ...
  renderer.setSize(bodyWidth, bodyHeight);
  camera.init(get.radian(45), get.radian(0), bodyWidth /2, bodyHeight / 2);
